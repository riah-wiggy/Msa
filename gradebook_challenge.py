...
#sum of all nmumbers/15

def main():
        
        for column in range(len(gradebook[0])):
             total = 0
        ...
        for column in range(len(gradebook)):
            total = 0
            for row in range(len(gradebook[column])):
                #print(gradebook[column][row])
                total = total + gradebook[column][row]
            #calculate average
            average = total/len(gradebook[column]) 
            print(f"Student {column + 1}: {average:.2f} ")
